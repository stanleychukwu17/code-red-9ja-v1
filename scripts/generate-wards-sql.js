...
async function main() {
  try {
    ...
    console.log(
      `Loaded ${wards.length} wards. Generating SQL schema and insert rows...`,
    );
    ...
    sql += `    lga_id INTEGER NOT NULL,\n`;
    sql += `    lga_name VARCHAR(255) NOT NULL,\n`;
    sql += `    state_id INTEGER NOT NULL,\n`;
    sql += `    state_name VARCHAR(255) NOT NULL\n`;
    sql += `);\n\n`;

    sql += `INSERT INTO wards (id, name, abbreviation, lga_id, lga_name, state_id, state_name) VALUES\n`;

    const valueRows = wards.map((ward) => {
      const escapedName = ward.name.replace(/'/g, "''");
      const escapedAbbreviation = ward.abbreviation.replace(/'/g, "''");
      const escapedLgaName = ward.lga_name.replace(/'/g, "''");
      const escapedStateName = ward.state_name.replace(/'/g, "''");
      return `    (${ward.id}, '${escapedName}', '${escapedAbbreviation}', ${ward.lga_id}, '${escapedLgaName}', ${ward.state_id}, '${escapedStateName}')`;
    });
    ...
    console.log(
      `Successfully generated ${wardsSqlPath} with ${wards.length} Wards.`,
    );
  } catch (error) {
