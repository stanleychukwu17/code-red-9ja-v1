...
async function main() {
  try {
    ...
    console.log(
      `Loaded ${units.length} polling units. Generating SQL schema and insert rows...`,
    );
    ...
    const valueRows = units.map((unit) => {
      ...
      return (
        ...
        `${num(unit.lga_id)}, ${esc(unit.lga_name)}, ` +
    ...
    console.log(
      `Successfully generated ${pollingUnitsSqlPath} with ${units.length} polling units.`,
    );
  } catch (error) {
