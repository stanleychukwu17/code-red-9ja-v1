...
export function TargetUpdatesCountDialog({
  open,
  onClose,
  onSuccess,
}: {
  ...
  const settingKey = "target_updates_count";
    ...
    mutationFn: async (values: typeof form.state.values) => {
      const res = await updateSystemSetting({
        data: {
          ...
          description: "The target number of updates an agent is expected to give",
        },
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[480px] p-0 rounded-2xl border-none shadow-2xl bg-white overflow-visible">
        <DialogHeader title="Update Target Updates Count" />
          ...
          >
            <DialogPadding className="space-y-4 pb-6">
              <p className="text-c-50 leading-[22px]">
                Set the target for how many updates polling agents are expected to give during the election day.
              </p>
