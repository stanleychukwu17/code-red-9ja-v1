...
const ROLE_LEVEL_OPTIONS: Record<string, { label: string; value: string }[]> = {
  ...
  user: [
    { label: "Party Agent", value: "partyagent" },
    { label: "User", value: "user" },
  ],
};
...
export function UserFormDialog({
  ...
  const form = useForm({
    defaultValues: {
      ...
      originCountryId: undefined as number | undefined,
  ...
  React.useEffect(() => {
    if (open) {
      if (mode === "update" && user) {
        ...
        form.setFieldValue("originCountryId", user.origin_country_id || 161);
        form.setFieldValue("originStateId", user.state_of_origin || undefined);
        form.setFieldValue("partyId", user.party_id || undefined);
        form.setFieldValue("email", user.email || "");
        form.setFieldValue("password", "");
        form.setFieldValue("role", user.role || "user");
        form.setFieldValue("roleLevel", user.role_level || "user");
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: any) => {
      ...
      if (mode !== "update" && !values.password) throw new Error("Password is required");
      ...
      try {
        ...
        if (partiesRes.success && partiesRes.data?.parties) {
          ...
          if (partyObj) {
            ...
            partyShortName = partyObj.short_name;
          }
        }
      } catch (err) {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <form.Field
                    ...
                    validators={{
                      onChange: ({ value }) =>
                        !value ? "Country of origin is required" : undefined,
                    }}
                    children={(field) => (
                      <SelectCountry
                        selectedId={
