...
import { ReferralPayTable } from "#/components/Tables";
import { Coins, Search, X, Users } from "lucide-react";
...
export const Route = createFileRoute(
  ...
  head: () => getPageHeader({ title: "Agent Payments - Referral Pay" }),
  component: ReferralPayPage,
});

type SubTab = "unpaid" | "paid";

function ReferralPayPage() {
  ...
  const fetchReferralPay = useServerFn(getReferralPayList);
  const executePaySingle = useServerFn(payReferralAgent);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetchReferralPay({
        data: {
  ...
  const items: ReferralPayItem[] = Array.isArray(data) ? data : [];

  const handlePaySingle = async (item: ReferralPayItem) => {
    try {
      setPayingId(item.id);
      const res = (await executePaySingle({ data: item.id })) as any;
      if (res?.success) {
        toast.success(`Referral payment of ${item.earned_amount} recorded for ${item.user_label || item.user_name}`);
        queryClient.invalidateQueries({ queryKey: ["referral-pay-list"] });
        queryClient.invalidateQueries({ queryKey: ["agent-payments-overview"] });
      } else {
        toast.error(res?.message || "Failed to record referral payment");
      }
    } catch (e: any) {
  ...
  const handlePayAllConfirm = async () => {
    try {
      ...
      if (res?.success) {
        toast.success(res?.message || `Successfully processed payments for all ${items.length} referring agents`);
    ...
    <Layout>
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 py-4 border-b border-neutral-100 dark:border-neutral-800/80">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* Pay All Button (visible on Unpaid tab and items exist) */}
          {activeTab === "unpaid" && (
            <button
              type="button"
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-base font-semibold text-neutral-900 dark:text-white">
              No {activeTab} referral payouts
            </p>
            <p className="text-sm text-neutral-500 max-w-sm mx-auto">
              {searchInput
                ? `No referrers matching "${searchInput}" found in this group.`
                : activeTab === "unpaid"
                  ? "All earned referral balances for this group have been paid."
                  : "No referrers have received payouts for this group yet."}
            </p>
          </div>
        ) : (
          <ReferralPayTable
      ...
      <PayAllModal
        ...
        title="Pay All Referring Agents"
        description={`Are you sure you want to mark referral earnings for all ${items.length} referring agents as paid for this election group?`}
      />
    </Layout>
  );
}
