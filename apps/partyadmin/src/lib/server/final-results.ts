 ...
 * Picks the narrowest scope in this order:
 *   ward → stateConstituency → lga → federalConstituency → senatorialDistrict → state → (nationwide)
 *
 * Returns: { candidate_results, candidate_results_live, votes_cast, valid_votes, ... }
 * The frontend should use `candidate_results_live` when isLive=true, else `candidate_results`.
 */
export const getElectionScopedFinalResult = createServerFn({ method: "GET" })
  .inputValidator(
    (data: {
      electionId: number;
  ...
  .handler(async ({ data }) => {
    try {
      ...
      if (data.lgaId) {
        params.append("lga_id", String(data.lgaId));
      }
      ...
      const url = `${API_URL.electionResultsScoped}?${params.toString()}`;
      ...
      return {
        success: false,
        message:
          "Failed to fetch election final result: " + (error as Error).message,
      };
    }
  });
