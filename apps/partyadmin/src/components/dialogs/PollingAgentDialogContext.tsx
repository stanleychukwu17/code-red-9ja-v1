...
const PollingAgentContext = React.createContext<PollingAgentContextType | null>(null);

export function usePollingAgentDialog() {
  const ctx = React.useContext(PollingAgentContext);
  if (!ctx) {
    throw new Error("usePollingAgentDialog must be used within PollingAgentDialogProvider");
  }
  return ctx;
}

export function PollingAgentDialogProvider({ children }: { children: React.ReactNode }) {
  const [activeApp, setActiveApp] = React.useState<ApplicationData | null>(null);
