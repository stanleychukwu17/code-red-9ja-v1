...
function ElectionDayComponent() {
	...
	const resolvedStats =
		electionStatsData?.data?.party_stats || electionStatsData?.data?.stats || {};
	const targets = electionStatsData?.data?.targets || electionStatsData?.data?.stats || {};
		...
		<DashboardLayout>
			...
			<div className="grid gap-6 lg:grid-cols-[2fr_1.2fr] items-start">
				...
				<div className="space-y-6">
					...
					<LeaderboardCardWrapper>
						{isResultLoading ? (
							<div className="p-6 text-center text-gray-400 text-sm">Loading results…</div>
							...
							sortedResults.slice(0, 3).map((item: any, index: number) => {
								...
								const partyLogo: string | undefined =
									item.party?.logo || item.party_logo || item.logo;
								const votes: number = Number(item.votes ?? item.vote_count ?? 0);
										...
										regionsWinningCount={
											selectedWardId
												...
												: selectedLGAId || selectedStateConstituencyId
													...
													: selectedStateId || selectedDistrictId || selectedFederalConstituencyId
				...
				{/* Right Hand Column */}
				<ElectionStatsSidebar stats={resolvedStats} targets={targets} isLoading={isStatsLoading} />
			</div>
...
function ResultTypeCardInfo({ isLive }: { isLive: boolean }) {
	const LabelDetails = ({
		...
		<div className={cn("text-[15px] space-y-2 text-primary", className)}>
			<p>
				<span className="font-bold">{isLive ? "Live" : "Final"} Results:</span> {title}
			</p>
			<p>
		...
		<div className="space-y-2.5 cursor-pointer group" onClick={onClick}>
			...
			<div className="flex gap-2 items-start px-0.5">
				...
				<div className="flex-1 min-w-0">
					<p className="font-medium text-xs text-c-90 truncate leading-tight">{title}</p>
					<div className="flex items-center justify-between gap-2 mt-0.5">
						<span className="text-[11px] text-c-50 truncate leading-none">{subtitle}</span>
						{showReportIcon && <ReportCubeIcon className="size-4 rounded-[2px] shrink-0" />}
					</div>
				</div>
			</div>
		</div>
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
				{items?.map((item, index) => {
					const rawImage =
						item.media_urls || [item.finalResult?.result_sheet_image_url].filter(Boolean);
					const resolvedImageSrc = Array.isArray(rawImage) ? rawImage[0] : rawImage;
					const resolvedTitle = item.polling_unit_name || `PU ${item.polling_unit_id || ""}`;
					const resolvedSubtitle = [item.lga_name, item.state_name].filter(Boolean).join(", ");
...
function UpdatesGallery({ isReport }: { isReport: boolean }) {
	...
	const items = updatesData?.pages.flatMap((page: any) => page.data?.updates || []) || [];
		...
		<>
			...
			{selectedItem && (
				<UpdateReel
					...
					onNext={handleNext}
					onPrev={handlePrev}
					hasNext={selectedIndex !== null && (selectedIndex < items.length - 1 || hasNextPage)}
...
function FinalResultsGallery({ activeParties }: { activeParties?: any[] }) {
	...
	const results = finalResultsData?.pages.flatMap((page: any) => page.data?.results || []) || [];
	const items = results;

	const selectedResult = selectedResultIndex !== null ? items[selectedResultIndex] : null;
		...
		<>
			...
			{selectedResult && (
				<FinalResultReel
					...
					onPrev={handlePrev}
					hasNext={
						selectedResultIndex !== null && (selectedResultIndex < items.length - 1 || hasNextPage)
					}
