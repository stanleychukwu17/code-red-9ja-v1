...
const getEligiblePollingUnitsForElection = `-- name: GetEligiblePollingUnitsForElection :many
SELECT 
    pu.id,
    pu.state_id,
...
type GetEligiblePollingUnitsForElectionRow struct {
	ID                    int32       `json:"id"`
...
func (q *Queries) GetEligiblePollingUnitsForElection(ctx context.Context, arg GetEligiblePollingUnitsForElectionParams) ([]GetEligiblePollingUnitsForElectionRow, error) {
	rows, err := q.db.Query(ctx, getEligiblePollingUnitsForElection, arg.ID, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetEligiblePollingUnitsForElectionRow
	for rows.Next() {
		var i GetEligiblePollingUnitsForElectionRow
		if err := rows.Scan(
			&i.ID,
			&i.StateID,
...
func (q *Queries) GetPollingUnitFinalResult(ctx context.Context, arg GetPollingUnitFinalResultParams) (ElectionPollingUnitFinalResult, error) {
	row := q.db.QueryRow(ctx, getPollingUnitFinalResult, arg.ElectionID, arg.PollingUnitID)
	var i ElectionPollingUnitFinalResult
	err := row.Scan(
		&i.ID,
		&i.ElectionID,
		&i.ElectionGroupID,
...
const refreshPollingUnitLiveResults = `-- name: RefreshPollingUnitLiveResults :exec
WITH live_counts AS (
  SELECT 
...
func (q *Queries) RefreshPollingUnitLiveResults(ctx context.Context, arg RefreshPollingUnitLiveResultsParams) error {
	_, err := q.db.Exec(ctx, refreshPollingUnitLiveResults,
		arg.ElectionID,
		arg.PollingUnitID,
		arg.ElectionGroupID,
		arg.StateID,
...
const rollupSingleElectionFinalResults = `-- name: RollupSingleElectionFinalResults :exec
WITH agg AS (
    SELECT 
        r.election_id,
        COALESCE(SUM(r.accredited_voters), 0) as accredited_voters,
...
const rollupSingleFederalConstituencyFinalResults = `-- name: RollupSingleFederalConstituencyFinalResults :exec
WITH agg AS (
    SELECT 
        r.election_id, l.federal_constituency_id, MIN(r.state_id) as state_id, MIN(l.senatorial_district_id) as senatorial_district_id,
        COALESCE(SUM(r.accredited_voters), 0) as accredited_voters,
...
func (q *Queries) RollupSingleFederalConstituencyFinalResults(ctx context.Context, arg RollupSingleFederalConstituencyFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleFederalConstituencyFinalResults, arg.ElectionID, arg.FederalConstituencyID)
	return err
}

const rollupSingleLGAFinalResults = `-- name: RollupSingleLGAFinalResults :exec
WITH agg AS (
    SELECT 
        r.election_id, r.lga_id, MIN(r.state_id) as state_id, MIN(l.senatorial_district_id) as senatorial_district_id, MIN(l.federal_constituency_id) as federal_constituency_id,
        COALESCE(SUM(r.accredited_voters), 0) as accredited_voters,
...
func (q *Queries) RollupSingleLGAFinalResults(ctx context.Context, arg RollupSingleLGAFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleLGAFinalResults, arg.ElectionID, arg.LgaID)
	return err
}

const rollupSingleSenatorialDistrictFinalResults = `-- name: RollupSingleSenatorialDistrictFinalResults :exec
WITH agg AS (
    SELECT 
        r.election_id, l.senatorial_district_id, MIN(r.state_id) as state_id,
        COALESCE(SUM(r.accredited_voters), 0) as accredited_voters,
...
func (q *Queries) RollupSingleSenatorialDistrictFinalResults(ctx context.Context, arg RollupSingleSenatorialDistrictFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleSenatorialDistrictFinalResults, arg.ElectionID, arg.SenatorialDistrictID)
	return err
}

const rollupSingleStateConstituencyFinalResults = `-- name: RollupSingleStateConstituencyFinalResults :exec
WITH agg AS (
    SELECT 
...
func (q *Queries) RollupSingleStateConstituencyFinalResults(ctx context.Context, arg RollupSingleStateConstituencyFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleStateConstituencyFinalResults, arg.ElectionID, arg.ID)
	return err
}

const rollupSingleStateFinalResults = `-- name: RollupSingleStateFinalResults :exec
WITH agg AS (
    SELECT 
...
func (q *Queries) RollupSingleStateFinalResults(ctx context.Context, arg RollupSingleStateFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleStateFinalResults, arg.ElectionID, arg.StateID)
	return err
}

const rollupSingleWardFinalResults = `-- name: RollupSingleWardFinalResults :exec
...
// =========================================================================
func (q *Queries) RollupSingleWardFinalResults(ctx context.Context, arg RollupSingleWardFinalResultsParams) error {
	_, err := q.db.Exec(ctx, rollupSingleWardFinalResults, arg.ElectionID, arg.WardID)
	return err
}
...
const updateCandidatesFromSingleFederalConstituencyElection = `-- name: UpdateCandidatesFromSingleFederalConstituencyElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleFederalConstituencyElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleFederalConstituencyElection, id)
	return err
}

const updateCandidatesFromSingleLGAElection = `-- name: UpdateCandidatesFromSingleLGAElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleLGAElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleLGAElection, id)
	return err
}

const updateCandidatesFromSingleNationwideElection = `-- name: UpdateCandidatesFromSingleNationwideElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleNationwideElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleNationwideElection, id)
	return err
}

const updateCandidatesFromSingleSenatorialDistrictElection = `-- name: UpdateCandidatesFromSingleSenatorialDistrictElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleSenatorialDistrictElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleSenatorialDistrictElection, id)
	return err
}

const updateCandidatesFromSingleStateConstituencyElection = `-- name: UpdateCandidatesFromSingleStateConstituencyElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleStateConstituencyElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleStateConstituencyElection, id)
	return err
}

const updateCandidatesFromSingleStateElection = `-- name: UpdateCandidatesFromSingleStateElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleStateElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleStateElection, id)
	return err
}

const updateCandidatesFromSingleWardElection = `-- name: UpdateCandidatesFromSingleWardElection :exec
UPDATE election_candidates ec
SET votes_count = v.votes, updated_at = NOW()
FROM (
  SELECT 
...
func (q *Queries) UpdateCandidatesFromSingleWardElection(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, updateCandidatesFromSingleWardElection, id)
	return err
}
...
func (q *Queries) UpsertPollingUnitFinalResult(ctx context.Context, arg UpsertPollingUnitFinalResultParams) (ElectionPollingUnitFinalResult, error) {
