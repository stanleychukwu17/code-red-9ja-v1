...
type StateConstituenciesService interface {
	CreateStateConstituency(ctx context.Context, name string, code string, lgaID int32, lgaName string, stateID int32, stateName string, senatorialDistrictID int32, senatorialDistrictName string, federalConstituencyID int32, federalConstituencyName string) (queries.StateConstituency, error)
	GetStateConstituencyByID(ctx context.Context, id int32) (queries.StateConstituency, error)
	UpdateStateConstituency(ctx context.Context, id int32, name string, code string, lgaID int32, lgaName string, stateID int32, stateName string, senatorialDistrictID int32, senatorialDistrictName string, federalConstituencyID int32, federalConstituencyName string) (queries.StateConstituency, error)
...
type CreateStateConstituencyRequest struct {
...
type UpdateStateConstituencyRequest struct {
	Name                  string  `json:"name"`
	Code                  *string `json:"code"`
	LgaID                 int32   `json:"lga_id"`
	StateID               int32   `json:"state_id"`
...
// @Accept       json
// @Produce      json
// @Param        request body CreateStateConstituencyRequest true "Create request payload"
...
func (h *Handler) CreateStateConstituency(w http.ResponseWriter, r *http.Request) {
	var req CreateStateConstituencyRequest
...
func (h *Handler) GetStateConstituency(w http.ResponseWriter, r *http.Request) {
...
// @Produce      json
// @Param        id   path  int  true  "State Assembly Constituency ID"
// @Param        request body UpdateStateConstituencyRequest true "Update request payload"
...
func (h *Handler) UpdateStateConstituency(w http.ResponseWriter, r *http.Request) {
	idStr := chi.URLParam(r, "id")
	id, err := strconv.ParseInt(idStr, 10, 32)
	if err != nil {
		h.utils.RespondError(w, http.StatusBadRequest, "Invalid state assembly constituency ID")
	...
	var req UpdateStateConstituencyRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.utils.RespondError(w, http.StatusBadRequest, "Invalid request payload")
		return
	}
...
func (h *Handler) DeleteStateConstituency(w http.ResponseWriter, r *http.Request) {
	idStr := chi.URLParam(r, "id")
	id, err := strconv.ParseInt(idStr, 10, 32)
	if err != nil {
		h.utils.RespondError(w, http.StatusBadRequest, "Invalid state assembly constituency ID")
...
// @Success      200                      {object}  GetStateConstituenciesResponse
// @Failure      500                      {string}  failed to fetch constituencies
// @Router       /state-constituencies [get]
func (h *Handler) GetStateConstituencies(w http.ResponseWriter, r *http.Request) {
	...
	if search != "" {
		...
		var filtered []queries.StateConstituency
		for _, c := range constituencies {
			...
			if strings.Contains(strings.ToLower(c.Name), searchLower) ||
				...
				strings.Contains(strings.ToLower(c.LgaName), searchLower) ||
				strings.Contains(strings.ToLower(c.FederalConstituencyName.String), searchLower) ||
	...
	var paginated []queries.StateConstituency
	hasMore := false
	nextCursor := ""
	...
	if startIndex < len(constituencies) {
		...
		paginated = []queries.StateConstituency{}
	}
