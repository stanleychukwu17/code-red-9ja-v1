...
type RegisterRequest struct {
	...
	OtherName      string `json:"other_name" validate:"omitempty,min=2,max=30"`
...
func (h *Handler) Register(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.RespondJSON(w, http.StatusBadRequest, map[string]string{"error": "Invalid date format for date_of_birth. Use YYYY-MM-DD"})
		return
	}
	...
	params := queries.CreateUserParams{
		...
		OtherName:      pgtype.Text{String: req.OtherName, Valid: req.OtherName != ""},
	...
	if err != nil {
		utils.RespondJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to create user: " + err.Error()})
		return
	}
