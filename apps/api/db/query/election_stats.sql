...
SELECT
  ...
  oa.pu_average_election_started_at, oa.pu_average_election_ended_at,
...
WITH epu_agg AS (
  SELECT
    ...
    SUM(pu_reports_count)                         AS reports_count,
    SUM(pu_updates_count)                         AS updates_count,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_average_election_started_at)))                 AS pu_pu_average_election_started_at,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_average_election_ended_at)))                   AS pu_pu_average_election_ended_at,
    COALESCE(AVG(pu_election_practice_test_readiness_percentage), 0) AS pu_pu_election_practice_test_readiness_percentage,
    ...
    SUM(pu_live_voters_referred_by_agent_count)         AS pu_pu_live_voters_referred_by_agent_count,
...
party_agg AS (
  SELECT
    ...
    to_timestamp(AVG(EXTRACT(epoch FROM pu_average_arrival_time))) AT TIME ZONE 'UTC' AS pu_pu_average_arrival_time,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_average_election_started_at))) AT TIME ZONE 'UTC' AS pu_pu_average_election_started_at,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_average_election_ended_at))) AT TIME ZONE 'UTC' AS pu_pu_average_election_ended_at,
    ...
    SUM(referrals)                         AS pu_pu_live_voters_referred_by_agent_count,
    COALESCE(AVG(avg_interval_seconds), 0) AS pu_pu_average_update_time_interval_in_seconds,
    COALESCE(AVG(readiness_pct), 0)        AS pu_pu_election_practice_test_readiness_percentage,
...
party_json AS (
  SELECT
    ...
    COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'pu_pu_average_arrival_time',               pu_pu_average_arrival_time,
      'pu_pu_average_election_started_at',        pu_pu_average_election_started_at,
      'pu_pu_average_election_ended_at',          pu_pu_average_election_ended_at,
      ...
      'pu_pu_average_update_time_interval_in_seconds', pu_pu_average_update_time_interval_in_seconds,
      'pu_pu_election_practice_test_readiness_percentage', pu_pu_election_practice_test_readiness_percentage,
      'pu_pu_live_voters_referred_by_agent_count', pu_pu_live_voters_referred_by_agent_count,
...
INSERT INTO election_group_wards (
  ...
  pu_pu_average_election_started_at, pu_pu_average_election_ended_at,
  pu_pu_election_practice_test_readiness_percentage,
  pu_final_results_uploaded_count, unique_pu_final_results_uploaded_count,
  pu_pu_live_voters_referred_by_agent_count,
...
SELECT
  ...
  a.pu_pu_average_election_started_at, a.pu_pu_average_election_ended_at,
  a.pu_pu_election_practice_test_readiness_percentage,
  a.pu_final_results_uploaded_count, a.unique_pu_final_results_uploaded_count,
  a.pu_pu_live_voters_referred_by_agent_count,
...
ON CONFLICT (election_group_id, ward_id) DO UPDATE SET
  ...
  pu_pu_average_election_started_at = EXCLUDED.pu_pu_average_election_started_at,
  pu_pu_average_election_ended_at = EXCLUDED.pu_pu_average_election_ended_at,
  pu_pu_election_practice_test_readiness_percentage = EXCLUDED.pu_pu_election_practice_test_readiness_percentage,
  pu_final_results_uploaded_count = EXCLUDED.pu_final_results_uploaded_count,
  unique_pu_final_results_uploaded_count = EXCLUDED.unique_pu_final_results_uploaded_count,
  pu_pu_live_voters_referred_by_agent_count = EXCLUDED.pu_pu_live_voters_referred_by_agent_count,
...
WITH src_agg AS (
  SELECT
    ...
    SUM(reports_count)                      AS reports_count,
    SUM(updates_count)                      AS updates_count,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_pu_average_election_started_at)))              AS pu_pu_average_election_started_at,
    to_timestamp(AVG(EXTRACT(epoch FROM pu_pu_average_election_ended_at)))                AS pu_pu_average_election_ended_at,
    COALESCE(AVG(pu_pu_election_practice_test_readiness_percentage), 0) AS pu_pu_election_practice_test_readiness_percentage,
    SUM(pu_final_results_uploaded_count)       AS pu_final_results_uploaded_count,
    SUM(unique_pu_final_results_uploaded_count) AS unique_pu_final_results_uploaded_count,
    SUM(pu_pu_live_voters_referred_by_agent_count)      AS pu_pu_live_voters_referred_by_agent_count,
...
party_expanded AS (
  SELECT
    ...
    (p.value->>'pu_pu_average_update_time_interval_in_seconds')::float    AS avg_interval_seconds,
    (p.value->>'pu_pu_election_practice_test_readiness_percentage')::float AS readiness_pct,
    (p.value->>'pu_pu_live_voters_referred_by_agent_count')::int          AS referrals,
      ...
      SELECT jsonb_agg(
        CASE
          ...
          THEN jsonb_set(
                 elem,
                 '{agents_count}',
                 to_jsonb(GREATEST(0, COALESCE((elem->>'agents_count')::int, 0) + sqlc.arg(delta)::int))
               )
          ELSE elem
        END
      )
        ...
        THEN jsonb_set(
               jsonb_set(
                 jsonb_set(
                   jsonb_set(
                     elem,
                     '{reports_count}',
                     to_jsonb(COALESCE((elem->>'reports_count')::int, 0) + sqlc.arg(reports_delta)::int)
                   ),
                   '{updates_count}',
                   to_jsonb(COALESCE((elem->>'updates_count')::int, 0) + sqlc.arg(updates_delta)::int)
                 ),
               ...
               to_jsonb(
                 CASE
                   ...
                   ELSE (
                     (COALESCE((elem->>'pu_average_update_time_interval_in_seconds')::numeric, 0.0) * (COALESCE((elem->>'updates_count')::numeric, 0) + COALESCE((elem->>'reports_count')::numeric, 0)))
...
SET
  ...
  parties = CASE
    ...
    ELSE parties || jsonb_build_object(
      ...
      'pu_pu_average_update_time_interval_in_seconds',       pu_pu_average_update_time_interval_in_seconds,
      'pu_pu_election_practice_test_readiness_percentage',   pu_pu_election_practice_test_readiness_percentage,
      'pu_pu_live_voters_referred_by_agent_count',           0,
...
SET
  ...
  pu_pu_average_election_started_at = s.pu_pu_average_election_started_at,
  pu_pu_average_election_ended_at = s.pu_pu_average_election_ended_at,
  pu_pu_election_practice_test_readiness_percentage = COALESCE(s.pu_pu_election_practice_test_readiness_percentage, 0),
  ...
  pu_pu_live_voters_referred_by_agent_count = COALESCE(s.pu_pu_live_voters_referred_by_agent_count, 0),
...
party_expanded AS (
  SELECT
    ...
    COALESCE(AVG(a.pu_election_practice_test_readiness_percentage), 0) AS readiness_pct,
