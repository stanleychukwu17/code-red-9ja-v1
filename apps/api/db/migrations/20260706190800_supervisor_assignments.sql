...
CREATE TABLE IF NOT EXISTS state_election_supervisors (
  ...
  role_type VARCHAR(50) DEFAULT 'state_supervisor' CHECK (role_type IN ('state_supervisor')),
...
CREATE TABLE IF NOT EXISTS lga_election_supervisors (
  ...
  election_group_id BIGINT REFERENCES election_groups(id) ON DELETE CASCADE NOT NULL,
  party_id BIGINT REFERENCES parties(id) ON DELETE CASCADE NOT NULL,
  role_type VARCHAR(50) DEFAULT 'lga_supervisor' CHECK (role_type IN ('lga_supervisor')),
  assigned_by BIGINT REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS ward_election_supervisors (
  ...
  election_group_id BIGINT REFERENCES election_groups(id) ON DELETE CASCADE NOT NULL,
  party_id BIGINT REFERENCES parties(id) ON DELETE CASCADE NOT NULL,
  role_type VARCHAR(50) DEFAULT 'ward_supervisor' CHECK (role_type IN ('ward_supervisor')),
  assigned_by BIGINT REFERENCES users(id) ON DELETE SET NULL,
